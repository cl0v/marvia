...
class SpiderWeb extends CustomPainter {
  ...
  double ss(double x) {
    return sqrt(pow(x, 2) + pow(x, 2)) - x * multiplier;
  }

  void canvasDrawLineStartBottomToRight(Canvas c, Offset center, double y) {
    final d = ss(y);
    c.drawLine(center + Offset(0, d), center + Offset(y / 2, y / 2),
        Paint()..strokeWidth = 2);
  }
  ...
  void canvasDrawLineStartLeftToTop(Canvas c, Offset center, double y) {
    final d = ss(y);
    c.drawLine(center + Offset(-d, 0), center + Offset(-y / 2, -y / 2),
        Paint()..strokeWidth = 2);
  }
